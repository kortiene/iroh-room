...
This directory holds the **frozen** golden-vector fixtures for every signed record
and domain-separated hash boundary produced by `iroh-rooms-v2-core` (issue #153,
spec `v2-signed-record-golden-vectors.md`, refs #134 §1 / §6.4).
...
- `v2-signed-records.json` — the single aggregate, hand-reviewable JSON file. Every
  byte sequence is lowercase hex with no `0x` prefix. Named hashes use the
  `blake3:<64-hex>` presentation form; raw digest hex is stored alongside where a
  byte-exact hash input/output is being pinned.
- The Rust test `../signed_records_golden.rs` carries the same values as compile-time
  constants and is the authoritative check; the JSON is the human-reviewable mirror.
