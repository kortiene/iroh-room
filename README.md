...
- **Small rooms only (≤5 members)**: the transport is a full QUIC mesh sized for the declared ≤5-peer ceiling (ADR-1, `PHASE-0-SPIKE.md`). The cap (`MAX_ACTIVE_MEMBERS = 5`) is enforced as a hard `RejectReason::RoomFull` reject on the 6th active join, and a near-cap warning (`room.active_members.near_cap`) plus an `active: <n>/5 (<k> slots remaining)` line in `room members --status` surface that the room is approaching the ceiling once it reaches 4 active members. The cap is a protocol invariant and is not configurable. Above the ceiling the failure mode before the reject was silent: at N=25 the system was measured to stop delivering messages entirely (idle `frames_sent=0`, `accepted=0`, a 661 MB inbound backlog) while every connectivity signal still read healthy. Do not run rooms larger than 5 participants.
