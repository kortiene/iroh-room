...
3. No user-reachable key rotation or forward secrecy after member removal. The
   protocol and the sync engine implement the #191 content-key rotation
   lifecycle in full — `content.encrypted` envelopes, per-epoch room keys, the
   `member.removed` rotation payload, `member.key_distribution`, and chunked
   key-history transfer — but no shipped surface triggers it: the CLI has no
   remove/rotate command and no production call site enables encrypted writes,
   so a CLI-operated room is plaintext with no opt-in.
...
| T26 | Unadmitted peer joins the gossip overlay and receives event bytes | High | The `gossip_overlay` feature adds a `GOSSIP_ALPN` whose `GossipProtocolHandler` consults the same `Arc<dyn Admission>` instance as the event-plane gate, closing the connection with `REJECT_CODE` at `accept()` time **before** the inner gossip handler runs (reject-before-bytes preserved); the per-room `TopicId` is derived from the public `room_id` and is a rendezvous point, not the boundary; the forwarding set stays ⊆ admitted device keys (issue #171 / #154, spec §4 D2) | The feature is default-off everywhere that ships: neither the supported CLI nor the SDK facade's `experimental` feature enables it, so shipped binaries register no `GOSSIP_ALPN` at all and this row describes no live attack surface today. The boundary is structural (verified by loopback reject tests) and applies the moment an operator opts in via `iroh-rooms-net/gossip_overlay`. Forwarding-visibility posture is unchanged vs full-mesh | Controlled |
| T27 | Removed device keeps receiving room fan-out over the gossip overlay | High | Two-layer self-standing revocation on the honest transport: (1) `GossipReconciler::reconcile` drops the local device's own mesh and blocks re-subscribe the moment the fold no longer marks self `Active` (the `desired_devices` set excludes self, so without this the reconciler's `revoked` diff could never observe self-removal); (2) the receiver task re-checks the **local** device's own admission at delivery time (`accept_delivered_frame`), dropping inbound frames while self is not `Admit` even if a mesh lingers (async topic-leave / stalled reconcile) | These close the leak for an honest-but-removed device whose transport runs the shipped code. A **malicious** removed device that patches its own transport can still subscribe to the public topic and read fan-out it can reach, and the #191 rotation lifecycle does **not** close that in any shipped configuration. The protocol and the sync engine implement rotation in full, but no shipped surface reaches it: the CLI has no member-removal or rotation command, `iroh-rooms-net` authors every `member.removed` with `rotation: None` (`manager.rs:687`, `node.rs:2395`/`:2548`/`:2779`), no production call site sets `SyncConfig::encrypted_content_writes` (`sync/config.rs:146-151`), and `WantKeyHistory` is served but never emitted. Content in a CLI-operated room is therefore plaintext and a malicious removed device reads it. Rotation takes effect only for an embedder driving `SyncEngine` / the experimental SDK tier directly, and for that deployment the post-removal bytes are ciphertext under a key the removed device never receives. T18 (withheld removal) still bounds how quickly the removal itself propagates | Partial for every shipped surface — CLI-operated rooms stay plaintext because nothing triggers rotation. Controlled for the malicious-reader case only where an embedder drives the engine's rotation API; the residuals are then T28/T29/T30 |
| T28 | Epoch key store is a new local secret | High | The `epoch → room_key` map is held only in memory (session) and persisted in `rooms.db` via `room_keys` table; at-rest protection is the same as the identity keys (ADR-0001 scope) | A read of `rooms.db` without the key store yields only ciphertext; a read with the key store exposes all covered history. Storage encryption is a production-blocking decision (D01) | Partial until storage encryption lands |
| T29 | Rotation latency / admin-offline stall | Medium | The engine *can* carry a rotation payload atomically in an admin `member.removed` (single publish), and a voluntary `member.left` folds immediately so access is revoked at once. But **no shipped surface authors either rotation payload**, and v1 fires **no** automatic rotation on the admin's next event — the departed device keeps decrypting new content until an operator explicitly rotates through the engine API | A removed device reads content until the removal + distribution propagates, and rotation stalls entirely if the single admin is offline. Small-room/online-peer scoping; no multi-admin or offline-rotation mechanism in v1 | Partial |
| T30 | Metadata leakage (incl. length) | Medium | The envelope (`event_type`/`inner_type`, `sender_id`, `device_id`, timing, the causal DAG) stays cleartext; AEAD preserves plaintext length apart from the 16-byte tag | An observer or removed device learns event existence, type, and an often-effectively-exact body size. Length-hiding padding is out of v1 scope (§12) | Partial |
...
- No user-reachable content-key rotation. The #191 rotation lifecycle is
  implemented in the protocol and the sync engine, but the shipped CLI has no
  command to trigger it and no configuration path enables encrypted writes, so
  CLI-operated rooms carry plaintext content (threat-model T27).
- No secure multi-device recovery.
- No full metadata privacy.
- Room size is capped at 5 active members in shipped builds.
