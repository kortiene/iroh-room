...
| T29 | Rotation latency / admin-offline stall | Medium | Admin removals carry the rotation payload atomically in the removal event (single publish); a voluntary `member.left` folds immediately and the admin's next event excludes the departed member from future epochs | A removed device reads content until the removal + distribution propagates, and rotation stalls entirely if the single admin is offline. Small-room/online-peer scoping; no multi-admin or offline-rotation mechanism in v1 | Partial |
