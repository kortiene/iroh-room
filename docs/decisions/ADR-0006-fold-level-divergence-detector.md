...
| `MemberRemoved` | **Yes** — effect `remove`. | This detector (invite + remove of one subject conflicts; two removes are same-effect ⇒ no fire). The optional `rotation` payload is a `MemberKeyDistribution`; its conflicts are owned by the independent D5a epoch-poisoning path (same-epoch), **not** this detector — see Follow-ups for the cross-epoch gap. |
...
- **Cross-epoch rotation conflicts on concurrent removals.** A `MemberRemoved` carries an optional `rotation` (`MemberKeyDistribution`); the divergence detector collapses every removal to the `remove` effect (key-distribution conflicts are D5a's domain, per the boundary above). D5a compares conflicting commitments only **within** one epoch, so two concurrent removals of one subject whose rotations target **different** epochs (or one with a rotation and one without) are not flagged by either mechanism. Decide whether to extend D5a to cross-epoch removal rotations, or surface them here.
- **Efficiency.** `admin_divergence` scans the held fold's nodes on every `recompute_completeness` (O(room history) per accepted event). Acceptable for v1 room sizes (≤5, soon 20–40 members; event counts in the hundreds-to-low-thousands) because the per-node work is a cheap `is_accepted` + content match, but it should be cached by `membership_projection_generation` (recompute only on membership change, not on every chat message) before Phase C's larger rooms.
