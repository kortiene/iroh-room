...
- [x] `crates/iroh-rooms-v2-core/` exists and is a root workspace member (root `Cargo.toml` `[workspace] members`).
- [x] `crates/iroh-rooms-v2-core/Cargo.toml` has `publish = false` (line 12).
- [x] The crate builds cleanly under workspace CI (`cargo build`/`cargo test -p iroh-rooms-v2-core --all-targets --all-features` green; 0 warnings).
- [x] The crate has no dependency on `iroh`, `iroh-blobs`, `iroh-gossip`, `tokio`, `rusqlite`, or runtime/store crates (deps limited to `ed25519-dalek`/`blake3`/`hex`/`zeroize`/`getrandom` + dev `proptest`; machine-checked by `tests/banned_dependencies.rs` via `cargo tree`).
...
- [x] Taxonomy completeness test covers every public rejection/error code (naming closure in `tests/taxonomy.rs`: `EXPECTED` ↔ `error::all_codes()` ↔ the enum agree, codes stable `snake_case`; plus a reachability gate `every_code_is_reachable_or_explicitly_blocked` proving every code is constructed by a public path in `src/` or is an explicit `BLOCKED_CODES` entry; frozen byte-exact exercise vectors for every reachable code live in `tests/signed_records_golden.rs` (#153), with `BLOCKED_CODES = ["wrong_domain"]` kept in sync across both).
- [ ] Independent audit of governance state machine and fork handling is complete and recorded (draft at `docs/audits/v2-governance-fork-audit-2026-07-30.md` — covers `governance/log/{fork,machine,authz}.rs`, all 14 safety invariants verified code+test, no live defect; **pending independent sign-off in §7**, after which this box flips).
