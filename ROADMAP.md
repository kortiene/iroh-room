...
| RB-SEC-010 | Complete an independent human/firm review of all shipped trust surfaces before cohort activation. | PRE | PRD v0.3 §13.5; production readiness | `iroh-room` / pending RB-GOV-004 | All implementation rows; RB-FC-002 | Published scope, findings, dispositions, and final-candidate delta | Independent human reviewer/firm | not-started | Yes |
...
| ID | Exact deliverable | Class | Source | Owner / issue | Dependencies | Completion evidence | Hardware / external role | Status | Cohort-blocking |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RB-PRD6-001 | Implement call signaling with `call.started` and `call.ended` room events. | PRE | PRD v0.3 §9.4/Phase 6 | `iroh-room` / pending RB-GOV-004 | RB-DEC-001; RB-PRD3 event framework | Schemas, authorization, vectors, migration, and reconnect tests | Protocol reviewer | not-started | Yes |
...
| RB-CPT-043 | Cockpit release-readiness checklist and honest limitations. | PRE | CPT-043 | `pi-rooms-cockpit` / pending | RB-CPT-041; all CPT/UP rows | Candidate checklist passes with no invented metrics | Release reviewer | blocked | Yes |
...
| RB-COH-007 | After material fixes, cut a replacement candidate, rerun affected FC gates, and restart/extend the cohort window when required. | COHORT | ADR-0012 | `iroh-room` / pending after findings | RB-COH-004 | Replacement tag, gate mapping, and dated window decision | Release owner + affected reviewers | deferred | No |
